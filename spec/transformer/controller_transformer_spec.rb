...
describe "Controllers Transformer should transform .rb correctly" do
  ...
  context "with alert message and new syntax" do
    ...
    it {should == [
      "redirect_to root_path, alert: t('some_controller.you_cannot_delete')",
      {"#{key_base}.you_cannot_delete" => "You cannot delete your account."}]
    }
  end
  ...
  context "with flash message" do
    ...
    it {should == [
      "flash[:notice] = t('some_controller.you_have') if some_boolean?",
      {"#{key_base}.you_have" => "You have successfully done something."}]
    }
  end
